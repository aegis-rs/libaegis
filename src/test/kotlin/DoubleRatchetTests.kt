...
class DoubleRatchetTests {
    ...
    private fun generateX3DHKeys(): X3DHKeys {
        ...
        val (aliceRoot, aliceChain) = RootKey.generateInitialRootKey(arrayOf(a1, a2, a3, a4))
        ...
        val (bobRoot, bobChain) = RootKey.generateInitialRootKey(arrayOf(b1, b2, b3, b4))
    ...
    fun testFiveMessageExchanges() {
        ...
        alice.ownRatchetKey = RatchetKey(aliceBC.public, aliceBC.private)
        val bob = DoubleRatchet(keys.bobRoot to keys.bobChain, receivedRatchetKey = alice.ownRatchetKey)
        val bobBC = java.security.KeyPairGenerator.getInstance("X25519", "BC").genKeyPair()
        bob.ownRatchetKey = RatchetKey(bobBC.public, bobBC.private)
