...
actual class DoubleRatchet private actual constructor(
    ...
    actual fun encrypt(message: ByteArray): Ciphertext {
        ...
        val header =
            Header(ownRatchetKey.publicKey.toJavaPublicKey().raw, lastSendingChainMessageAmount, sendingMessageNumber)
    ...
    actual fun decrypt(ciphertext: Ciphertext): ByteArray {
        ...
        if (!headerPublicKey.toJavaPublicKey().encoded.contentEquals(receivedRatchetKey?.publicKey?.toJavaPublicKey()?.encoded)) {
    ...
    actual override fun close() = this.destroy()
